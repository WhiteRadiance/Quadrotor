...
void SPI1_Init(void)//spi1_sclk(PA5),spi1_miso(PA6),spi1_mosi(PA7),其CS使用软件管理
{
	...
	SPI_InitStructure.SPI_Direction = SPI_Direction_1Line_Tx;			//设置SPI为单线只写(单工)
#else
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_16;	//设置SPI波特率 4.5M, SPI1=72M/16,SPI23=36/16,注意Vmax=18M
	SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;					//指定数据传输从MSBit位开始
...
{	
	u8 retry=0;
#if SPI1_1Line_TxOnly_DMA
	while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET)	//发送缓存  空标志位
	{
		retry++;
	...
	while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_RXNE) == RESET)	//接受缓存  非空标志位
	{
		retry++;
		if(retry>200) return 0;
	}
...
void SPI2_Init(void)//spi2_sclk(PB13),spi2_miso(PB14),spi2_mosi(PB15),其CS使用软件管理
{
	...
	SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;			//在第一个时钟沿采样
	SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;							//设置NSS信号由软件控制
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4;	//设置SPI波特率 9M, SPI23=36M/4,SPI1=72M/4,注意Vmax=18M
	SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;					//指定数据传输从MSBit位开始
	SPI_InitStructure.SPI_CRCPolynomial = 5;				//CRC校验多项式(>=1就行)
...
{		
	u8 retry=0;				 	
	while(SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE) == RESET)	//发送缓存  空标志位
	{
		retry++;
		if(retry>150) return 0;
	}			  
	SPI_I2S_SendData(SPI2, TxData);		//通过外设SPIx发送一个数据
	retry=0;

	while(SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_RXNE) == RESET)	//接受缓存  非空标志位
	{
		retry++;
		if(retry>150) return 0;
	}	  						    
