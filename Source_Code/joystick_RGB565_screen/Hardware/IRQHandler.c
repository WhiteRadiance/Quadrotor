...
void EXTI9_5_IRQHandler(void)						// key1(PB8)
{
	if(EXTI_GetITStatus(EXTI_Line8)!=RESET){
		...
		if(key1==0)	//再次判断(低电平)---现在非常完美!
			left_key_value = 1-left_key_value;		//only case 1 and 0
		else ;
	}
...
void EXTI15_10_IRQHandler(void)						// key2(PB11), key4_user2(PB10)
{
	if(EXTI_GetITStatus(EXTI_Line11)!=RESET){		//判断PB11
		...
		if(key2==0)	//再次判断(低电平)
			...
			right_key_value = 1-right_key_value;	//0x00, 0x01
		else ;
		...
		delay_ms(8);//消抖
		if(key4==0)	//再次判断(低电平)
			user2_key_value = 1-user2_key_value;	//0x00, 0x01
		else ;
...
void EXTI3_IRQHandler(void)							// key3_user1(PC3)
{
	if(EXTI_GetITStatus(EXTI_Line3)!=RESET){
		delay_ms(8);//消抖
		if(key3==0)	//再次判断(低电平)
			user1_key_value = 1-user1_key_value;	//0x00, 0x01
		else ;
	}
