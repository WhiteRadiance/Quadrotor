...
//[7]:0,不补零; 1,补0.
//[6:1]:保留
//[0]:0,非叠加显示(先擦在写); 1,叠加显示(不擦的情况下直接写字符).
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				...
				else LCD_ShowChar(x+(size/2)*t,y,' ',size,mode&0X01);//把' '改成'0',则每一位都会显示0,而不是只有末位是0。
 				continue;
			}else enshow=1; 
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,const u8 size,u8 *p)
{         
	u8 x0 = x;
	width += x;
	height += y;
