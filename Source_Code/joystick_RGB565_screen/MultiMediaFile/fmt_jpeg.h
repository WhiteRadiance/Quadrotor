 ...
 * JPEG解码流程: 将码流还原成RLE/Huffman编码 => 还原为矩阵 => 反Zigzag量化 => Inv-DCT => 拼接MCU
 */

typedef struct s_DQT {
	u16		label;			//Define Quantization Table = 0xFFDB
	u16		dqtlen;			//0x43 = 2+1+64=67
	u8		dqtacc_id;		//acc=[7:4] 0:8bit, 1:16bit, id=[3:0] #0-3 (2 tables almost)
...
//static HuffType* rd_HuffTbl_Y_DC = 0;	//size <= 16
//static HuffType* rd_HuffTbl_Y_AC = 0;	//size = 256
//static HuffType* rd_HuffTbl_UV_DC = 0;	//size <= 16
//static HuffType* rd_HuffTbl_UV_AC = 0;	//size = 256

//static HuffType rd_HuffTbl_Y_AC[256] = {0};
//static HuffType rd_HuffTbl_UV_AC[256] = {0};


//static u16	max_first_Lumi_DC[17] = { 0 }, max_first_Lumi_AC[17] = { 0 }, max_first_Chromi_DC[17] = { 0 }, max_first_Chromi_AC[17] = { 0 };
//static u8*	HuffTbl_len_Y_DC[17] = { 0 }, * HuffTbl_len_Y_AC[17] = { 0 }, * HuffTbl_len_UV_DC[17] = { 0 }, * HuffTbl_len_UV_AC[17] = { 0 };

static int		MCU_Y[8][8] = { 0 };
static int		MCU_U[8][8] = { 0 };
static int		MCU_V[8][8] = { 0 };
//static double		res_IDCT_YUV[8][8] = { 0.0 };
//static double		res_afterInvQ_YUV[8][8] = { 0.0 };
//static int		rle_YUV[128] = { 0 };
//static int		mat_YUV[8][8] = { 0 };	//由rle直接恢复出的原始矩阵,还没有反量化和反Zigzag



u16 	rgb888_to_565(u8 R8, u8 G8, u8 B8);

int		round_double(double val);
u16		rd_BigEnd16(u16 val);
void	rd_SOF0_2get_size(FIL* fp, u8* pY_sample, u16* imw, u16* imh);


void	Inverse_DCT(double res[][8], double block[][8]);
//void 	Build_DCHuffman_Table(const u8* nr_codes, const u8* std_tbl, HuffType* huffman_tbl, u16* max_inThisLen, u8* huff_len_tbl[], u8 IsLumi);
